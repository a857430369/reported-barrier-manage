<template>
  <el-form ref="ruleForm"
    ...
    <el-row>
      <el-col :span="(item.valueType == 'textarea' || item.valueType == 'remark' || item.valueType == 'remoteQuery') ? 22 : 11"
              v-for="(item,index) in showform"
              :key="index">
        <!-- 普通表单-开始 -->
        <el-form-item v-if="item.valueType != 'remoteQuery'"
        ...
        <el-form-item v-else
          ...
          <div class="remote-data vv-992c">
            <el-form :model="item.fieldValue" :inline="false">
              <el-form-item :label="key + ': '" v-for="key in Object.keys(item.fieldValue)" :key="key">
                {{ item.fieldValue[key] }}
              </el-form-item>
            </el-form>
          </div>
...
export default {
  ...
  methods: {
    getFormData() {
      this.$api.query.listNeedValueByFormCode({formCode: this.orderCode}).then(res => {
