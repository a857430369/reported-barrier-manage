<template>
  <div style="margin-top: 5px">
    <el-container style="height: 100%">
      <!-- 侧边导航栏 -->
      <el-card :style="{ height: screenHeight-20 + 'px', width:'280px',margin: '10px auto' }">
        <!-- 导航栏数据 -->
        <div :style="{ height: screenHeight-70 + 'px'}">
          
          <h2  @contextmenu.prevent="hadleContextMenu($event)" class="allFile" style="text-align: center;margin-bottom: 30px">
            <span 
              ...
              <el-tree
                ...
                <span class='custom-tree-node' slot-scope="{ node, data }">
                  ...
                  <span :class="node.isCurrent ? 'hightLine' : 'none-hightLine'">
                    <i :class="data.children.length == 0 ? 'el-icon-folder':'el-icon-folder-opened'"></i>
          ...
          <el-card class="header">
            <div class="allButton">
              ...
              <div class="groupButton">
                <el-input style="width:300px" v-model="fileName" placeholder="请输入文件名称" clearable><i slot="prefix" class="el-input__icon el-icon-search"></i></el-input>
                <el-button @click="screachName" type="primary" style="margin-left: 5px;margin-right: 5px" icon="el-icon-search">搜索</el-button>
                <!-- 可放图标 -->
            ...
            <el-card class="mainTable">
              ...
              <div v-if="tableFlag">
                <el-table
                  ...
                  label="操作">
                    <template slot-scope="scope">
                      <el-button type="primary" size="mini" @click="handleRemove(scope.row)"><i class="el-icon-more-outline">移动到</i></el-button>
                      <el-button type="primary" size="mini" @click="handleDownload(scope.row)"><i class="el-icon-download">下载</i></el-button>
              ...
              <div v-if="imageFlag" class="iconHieght">
                ...
                @change="handleCheckedChange">
                  <el-checkbox style="text-align: center"
                    v-for="city in fileData" 
                    :label="city.fileId"
                    :key="city.fileId">
                    <el-card :class="handleChecked(city.fileId)?'chckedOn':'chckedFalse'">
                    <img
                      :src="require('@/assets/file_images/'+getImage(city.filePath)+'.png')"
                      width="50px"
                      height="50px"/><br/>
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="onSubmit('ruleForm')">修 改</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog
      ...
      :before-close="handleClose">
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="onSubmit('ruleMenuForm')">修 改</el-button>
        </span>
    </el-dialog>
    ...
    <el-dialog
      ...
      <div class="removeDialog">
        <el-form :rules="rules" :model="form">
          ...
          <el-tree class="getTest" :data="MenuOptions" :props="defaultProps" @node-click="handlieClikeCode" node-key="menuCode" :default-expanded-keys="['0']">
            <span class='custom-tree-node' slot-scope="{ node, data }">
              <!-- 菜单名称图标 -->
              <span :class="node.isCurrent ? 'hightLine' : 'none-hightLine'">
                <i :class="data.children.length == 0 ? 'el-icon-folder':'el-icon-folder-opened'"></i>
...
export default {
  ...
  watch:{
    ...
    multipleSelection: function (newVal,oldVal) {
      if(newVal.length === 0){
        ...
        this.isIndeterminate = newVal.length > 0 && newVal.length < this.fileData.length;
  ...
  methods: {
    ...
    handleCheckedChange(value) {
      ...
      this.isIndeterminate = checkedCount > 0 && checkedCount < this.fileData.length;
    },
    // 获取勾选数组
    handleSelection(val) {
    ...
    getImage(img) {
      ...
      // 视频格式
      var videoList = ['avi', 'mov', 'qt', 'asf', 'mp4', 'rmvb', 'rm', 'divx', 'mpg', 'mpeg', 'mpe', 'wmv', 'mkv', 'vob']
    ...
    handleDownload(val) {
      this.beforeHandleButton()
      this.$api.file.downloadExcel(val.fileId).then((res) => {
    ...
    // 获取菜单--树形结构
    async getMenuAll() {
      await this.$api.fileMenu.getMenuAll().then((res) => {
