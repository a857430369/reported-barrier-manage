  ...
  >
    ...
    <el-table-column label="操作" header-align="center" align="center">
      <template slot-scope="main">
        <el-button
          @click="returnVerify(main.row)"
          v-if="
            isAutoVerify &&
              main.row.operCode !=
                ($store.state.user.isTest
                  ? $store.state.user.operCode
                  : $storage.getSession('account').operCode)
          "
          type="primary"
          size="mini"
          >退回</el-button
...
export default {
  ...
  methods: {
    getVerifyData() {
      ...
      this.$api.people
        .listNeedPlanAcc({ formCode: this.orderCode })
        .then((res) => {
    ...
    returnVerify(data) {
      ...
      let del = () => {
        let params = {
          ...
          operCode: this.$store.state.user.isTest
